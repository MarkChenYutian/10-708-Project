...
from ..Interface    import StereoInertialFrame, StereoFrame, StereoData, IMUData
...
class TartanAir_StereoSequence(SequenceBase[StereoFrame]):
    ...
    def __init__(self, config: SimpleNamespace | dict[str, Any]):
        ...
        self.lcam_time   = (np.load(str(Path(cfg.root, "imu", "cam_time.npy"))) * 1_000_000).astype(np.int64)
...
class TartanAirIMULoader(Dataset[IMUData]):
    def __init__(self, imuPath: Path) -> None:
        ...
        self.timestamp = torch.tensor(
            np.load(str(Path(imuPath, "imu_time.npy"))) * 1_000_000, dtype=torch.long
        ).unsqueeze(0)  # (1, N,)
        ...
        # Camera alignment
        self.camtime = torch.tensor(np.load(str(Path(imuPath, "cam_time.npy"))) * 1_000_000, dtype=torch.long)  # (M,)
    ...
    def frameRangeQuery(self, start_frame, end_frame) -> IMUData:
        ...
        return IMUData(
            ...
            gt_rot=self.gt_rot[:, start_imu_idx:end_imu_idx],           # type: ignore
            
            init_pos=self.gt_pos[:, start_imu_idx : start_imu_idx + 1],
            init_vel=self.gt_vel[:, start_imu_idx : start_imu_idx + 1],
            init_rot=self.gt_rot[:, start_imu_idx : start_imu_idx + 1], # type: ignore
        )
    ...
    def __getitem__(self, index) -> IMUData:
        ...
        return IMUData(
            ...
            gt_rot=self.gt_rot[:, index : index + 1],   #type: ignore
            
            init_pos=self.gt_pos[:, index : index + 1],
            init_vel=self.gt_vel[:, index : index + 1],
            init_rot=self.gt_rot[:, index : index + 1], #type: ignore
        )
