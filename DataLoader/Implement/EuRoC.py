...
from ..Interface import StereoData, IMUData, StereoFrame, StereoInertialFrame
...
class EuRoC_Sequence(SequenceBase[StereoInertialFrame]):
    ...
    def __init__(self, config: SimpleNamespace | dict[str, Any]) -> None:
        ...
        self.stereo_seq = self.stereo_seq.clip(start_idx=self.imu_seq.idx_start_cam, end_idx=self.imu_seq.idx_end_cam)
        self.offset     = self.imu_seq.idx_start_cam
        super().__init__(len(self.stereo_seq))
    ...
    def __getitem__(self, local_index: int) -> StereoInertialFrame:
        ...
        if index == 0:
            return StereoInertialFrame(
                ...
                imu=self.imu_seq[self.imu_seq.idx_imu_align_start]
            )
        else:
            return StereoInertialFrame(
                idx=[local_index],
                gt_pose=stereo_frame.gt_pose,
                stereo=stereo_frame.stereo,
                imu=self.imu_seq.frameRangeQuery(self.offset + index - 1, self.offset + index)
            )
...
class EuRoC_StereoSequence(SequenceBase[StereoFrame]):
    ...
    def __getitem__(self, local_index: int) -> StereoFrame:
        ...
        return StereoFrame(
            ...
            gt_pose= None if self.gt_pose_data is None else cast(pp.LieTensor, self.gt_pose_data[index])
        )
...
def load_EurocGTPose(csv_file_path: Path, cam_time: np.ndarray) -> tuple[pp.LieTensor, np.ndarray]:
    ...
    bodyPose_SE3 = interpolate_pose(pose_SE3, torch.tensor(pose_time), torch.tensor(cam_time[cam_time_mask]))
...
class EurocIMULoader(Dataset[IMUData]):
    def __init__(self, imu_path: Path) -> None:
        ...
        gt_time = rawGT[:, 0:1]
        self.timestamp = torch.tensor(rawIMU[..., 0:1], dtype=torch.long)
    ...
    def frameRangeQuery(self, start_frame, end_frame) -> IMUData:
        ...
        return IMUData(
            ...
            init_pos=self.gt_pos[start_imu_idx:end_imu_idx],
            init_vel=self.gt_vel[start_imu_idx:end_imu_idx],
            init_rot=cast(pp.LieTensor, self.gt_rot[start_imu_idx:end_imu_idx]),
        )
