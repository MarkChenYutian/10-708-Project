...
class StereoData(Collatable):
    ...
    baseline: list[float]   # Baseline (m) between left and right camera, len(list) = B
    time_ns : list[int]     # Time (ns) of data received, len(list) = B
    height: int             # H
    width : int             # W
    
    @property
    def frame_ns(self) -> float:
    ...
    def frame_baseline(self) -> float:
        assert len(self.baseline) == 1, "Can only use frame_baseline on unbatched data"
...
class IMUData(Collatable):
    ...
    gt_vel: torch.Tensor | None   = None      # torch.float32 of shape BxNx3
    gt_pos: torch.Tensor | None   = None      # torch.float32 of shape BxNx3
    gt_rot: pp.LieTensor | None   = None      # torch.float32 of shape BxNx4, pp.SO3 rotation.
    
    # Initial condition for IMU preintegration
    init_vel: torch.Tensor | None = None      # torch.float32 of shape Bx1x3
    init_pos: torch.Tensor | None = None      # torch.float32 of shape Bx1x3
    init_rot: pp.LieTensor | None = None      # torch.float32 of shape Bx1x4, pp.SO3 rotation.
