...
from Utility.Plot import plot_depth_calibcurve
...
def evaluate_depthcov(depth: IStereoDepth, seq: SequenceBase[StereoFrame], max_depth: float = 80.) -> DepthCovPerformance:
    ...
    for frame in ColoredTqdm(seq, desc="Evaluate DepthCov"):
        ...
        uncertainty_masks = [
            torch.logical_and(
                est_out.cov > DepthCovPerformance.uncertainty_bin[idx],
                est_out.cov < DepthCovPerformance.uncertainty_bin[idx + 1]
            )
            for idx in range(len(DepthCovPerformance.uncertainty_bin) - 1)
        ]
        ...
        results.append(DepthCovPerformance(
            ...
            uncertainty_avg   = [error2[:, 0][uncertainty_masks[idx][:, 0]].nanmedian().item() for idx in range(len(uncertainty_masks))],
        ))
    
    mean_cov = DepthCovPerformance.mean(results)
    plot_depth_calibcurve(mean_cov, "output.png")
    return mean_cov
