...
@pytest.mark.parametrize(["file_name", "data", "expect_ate", "expect_rte", "expect_roe"], [
    ...
    ("./Scripts/UnitTest/assets/test_config/MACVO.yaml", "./Scripts/UnitTest/assets/test_sequence/TartanAir2_abs_P000", 0.002, 0.0025, 0.045),      # at 2024 Sept. 22
])
...
@pytest.mark.local
@pytest.mark.parametrize(["file_name", "data", "expect_ate", "expect_rte", "expect_roe"], [    
    # Performance Test for AOT (notice that TRT module causes minor performance reduction on ATE metrics)
    ("./Scripts/UnitTest/assets/test_config/MACVO_aot.yaml", "./Scripts/UnitTest/assets/test_sequence/TartanAir2_abs_P000", 0.0025, 0.0025, 0.045),
])
def test_macvo_trt_performance(file_name: str, data: str, expect_ate: float, expect_rte: float, expect_roe: float):
    seq    = TartanAirV2_StereoSequence(dict(
        root=data,
        compressed=True, gtFlow=False, gtDepth=False, gtPose=True,
    ))
