...
class NoCovariance(IObservationCov):
    ...
    """
    def estimate(self, frame: StereoData, kp: torch.Tensor, depth_est: IStereoDepth.Output, match_est: IMatcher.Output | None, depth_cov: torch.Tensor | None, flow_cov: torch.Tensor | None) -> torch.Tensor:
        N = kp.size(0)
...
class MatchCovariance(IObservationCov):
    ...
    def estimate(self, frame: StereoData, kp: torch.Tensor, depth_est: IStereoDepth.Output, match_est: IMatcher.Output | None, depth_cov: torch.Tensor | None, flow_cov: torch.Tensor | None) -> torch.Tensor:
        ...
        if has_flow_cov_flag:
            ...
            PLTVisualizer.visualize_depth("depth", depth_est.depth)
            if depth_est.cov is not None:
                PLTVisualizer.visualize_depthcov("depth_cov", depth_est.cov)
...
class LearningCovariance(IObservationCov):
    def __init__(self, config: SimpleNamespace):
        super().__init__(config)
        from ..Network.CovarianceModifier import ICovarianceModifier
        
        self.modifier  = ICovarianceModifier.instantiate(config.modifier.type, config.modifier.args)
        if self.config.eval_mode: self.modifier.eval()
        else: self.modifier.train()
    ...
    def is_valid_config(cls, config: SimpleNamespace | None) -> None:
        ...
        ICovarianceModifier.is_valid_config(config)
        cls._enforce_config_spec(config, {
            "match_cov_default": lambda f: isinstance(f, float),
            "min_flow_cov": lambda f: isinstance(f, float),
            "eval_mode": lambda b: isinstance(b, bool)
        })
    
    def estimate(self, frame: StereoData, kp: torch.Tensor, depth_est: IStereoDepth.Output, match_est: IMatcher.Output | None, depth_cov: torch.Tensor | None, flow_cov: torch.Tensor | None) -> torch.Tensor:
        network_input = self.modifier.Input(
            K          =frame.K,
            keypoint   =kp,
            image      =frame.imageL,
            depth      =depth_est.depth,
            depth_cov  =depth_est.cov,
            flow       =None if match_est is None else match_est.flow,
            flow_cov   =None if match_est is None else match_est.cov ,
        )
        learn_cov, _ = self.modifier(network_input)
        return learn_cov
...
class Modifier_Diagonalize(IObservationCov):
    ...
    def estimate(self, frame: StereoData, kp: torch.Tensor, depth_est: IStereoDepth.Output, match_est: IMatcher.Output | None, depth_cov: torch.Tensor | None, flow_cov: torch.Tensor | None) -> torch.Tensor:
        covs = self.submodule.estimate(frame, kp, depth_est, match_est, depth_cov, flow_cov)
        for i in range(3):
            for j in range(3):
                if i == j: continue
                covs[..., i, j] = 0.
...
class Modifier_Learning(IObservationCov):
    """
    This class is a wrapper (modifier) to another IObservationCov model.
    
    On every call, it will forward everything to the internal model and update the predicted 
    3x3 covariance blocks with the result from an learning-based module.
    """
    def __init__(self, config: SimpleNamespace):
        from ..Network.CovarianceModifier import ICovarianceModifier
        
        super().__init__(config)
        self.submodule = IObservationCov.instantiate(config.submodule.type, config.submodule.args)
        self.modifier  = ICovarianceModifier.instantiate(config.modifier.type, config.modifier.args)
        
        if isinstance(self.config.checkpoint, str):
            ckpt = torch.load(self.config.checkpoint, map_location=self.config.device, weights_only=True)
            self.modifier.load_state_dict(ckpt)
            Logger.write("info", f"Load checkpoint at {self.config.checkpoint} for covariance modifier.")
    ...
    def is_valid_config(cls, config: SimpleNamespace | None) -> None:
        ...
        assert config is not None
        IObservationCov.is_valid_config(config.submodule)
        ICovarianceModifier.is_valid_config(config.modifier)
        cls._enforce_config_spec(config, {
            "match_cov_default": lambda f: isinstance(f, float),
            "min_flow_cov": lambda f: isinstance(f, float),
            "eval_mode": lambda b: isinstance(b, bool),
    ...
    def estimate(self, frame: StereoData, kp: torch.Tensor, depth_est: IStereoDepth.Output, match_est: IMatcher.Output | None, depth_cov: torch.Tensor | None, flow_cov: torch.Tensor | None) -> torch.Tensor:
        orig_cov  = self.submodule.estimate(frame, kp, depth_est, match_est, depth_cov, flow_cov)
        ...
        delta_cov, score = self.modifier(network_input)
        return orig_cov * (1 - score) + delta_cov * score
