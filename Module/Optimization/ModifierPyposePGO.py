...
class ModifierPyPoseTwoFramePGO(IOptimizer[PyposeGraphInput, PyposeContext, PyposeGraphOutput]):
    """
    ...
    def _get_graph_data(self, global_map: TensorMap, frame_idx: list[int]) -> PyposeGraphInput:
        ...
        return PyposeGraphInput(frame2opt, edges, self.config.device)
    ...
    def _optimize(context: PyposeContext, graph_data: PyposeGraphInput) -> tuple[PyposeContext, PyposeGraphOutput]:        
        with Timer.CPUTimingContext("TwoframePGO"), Timer.GPUTimingContext("TwoframePGO", torch.cuda.current_stream()):
            ...
            optimizer = GN(graph.double(), solver=context["solver"],
