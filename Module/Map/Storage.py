...
from typing_extensions import Callable, overload, Literal, cast
...
class BatchFrame:
    ...
    def __init__(self, K: torch.Tensor, pose: pp.LieTensor, obs_range: torch.Tensor,
                 flag: torch.Tensor, T_BS: pp.LieTensor, 
                 quality: torch.Tensor, frame_idx: torch.Tensor | None = None,
                 device: torch.device = MAP_DEVICE) -> None:
        self.K                  = K
        self.pose: pp.LieTensor = pose
    ...
    def __getitem__(self, slice) -> "BatchFrame":
        return BatchFrame(
            ...
            cast(pp.LieTensor, self.T_BS.__getitem__(slice)),
...
class BatchObservationStorage:
    def __init__(self) -> None:
        ...
        self.cov_pixel_uv = AutoScalingTensor((self.INIT_SIZE, 3), grow_on=0, dtype=torch.float)
...
class BatchFrameStorage:
    ...
    def __init__(self) -> None:
        ...
        # Stores T_BS
        self.T_BS = AutoScalingTensor((self.INIT_SIZE, 7), grow_on=0, dtype=torch.float)
    ...
    def __setitem__(self, slice, val: BatchFrame):
        ...
        self.T_BS.__setitem__(slice, val.T_BS)
    ...
    @property
    def body_pose(self) -> pp.LieTensor:
        T_BS = pp.SE3(self.T_BS.tensor)
        P_S  = pp.SE3(self.pose.tensor)
        return T_BS @ P_S @ T_BS.Inv()
