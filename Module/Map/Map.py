...
class TensorMap:
    ...
    def add_frame(self, K: torch.Tensor, pose: pp.LieTensor, num_obs: int, T_BS: pp.LieTensor,
        ...
        return self.frames.push(BatchFrame(
            K=K,
            pose=cast(pp.LieTensor, pose.unsqueeze(0) if pose.ndim == 1 else pose),
            obs_range=torch.tensor([[obs_idx_start, obs_idx_start + num_obs]]),
    ...
    def save_poses(self, file_path: Path | str, frame: Literal["body", "sensor"]):
        match frame:
            case "body":
                poses = self.frames.body_pose.numpy()
            case "sensor":
