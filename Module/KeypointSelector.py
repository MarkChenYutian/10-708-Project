...
class SelectorCompose(IKeypointSelector):
    ...
    def select_point(self, frame: StereoData, numPoint: int, depth_est: IStereoDepth.Output, match_est: IMatcher.Output | None) -> torch.Tensor:
        keypoints = []
        for selector, weight in zip(self.selectors, self.weight):
            keypoints.append(selector.select_point(frame, int(numPoint * weight), depth_est, match_est))
...
class CovAwareSelector(IKeypointSelector):
    ...
    def select_point(self, frame: StereoData, numPoint: int, depth_est: IStereoDepth.Output, match_est: IMatcher.Output | None) -> torch.Tensor:
        ...
        depth_map     = depth_est.depth.to(self.config.device)
        depth_cov_map = depth_est.cov.to(self.config.device)
        ...
        depth_cov_thresh = min(self.config.max_depth_cov, depth_cov_map[quality_nms].nanmedian().item() * 1.5)
        ...
        if depth_est.mask is not None:
            point_mask = torch.logical_and(point_mask, depth_est.mask.to(point_mask.device))
...
class CovAwareSelector_NoDepth(IKeypointSelector):
    ...
    def select_point(self, frame: StereoData, numPoint: int, depth_est: IStereoDepth.Output, match_est: IMatcher.Output | None) -> torch.Tensor:
        if match_est is None or match_est.cov is None:
            return self.fallback_grid_selector.select_point(frame, numPoint, depth_est, match_est)
        else:
            flow_cov_map = match_est.cov.to(self.config.device)
