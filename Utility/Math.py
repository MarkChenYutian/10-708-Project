...
#     num_kernel = 10
#     sig_xx, sig_yy = 10., 10.
#     cov_2x2_tensor = torch.stack([
#         torch.tensor([[sig_xx, rho * math.sqrt(sig_xx) * math.sqrt(sig_yy)], 
#                       [rho * math.sqrt(sig_xx) * math.sqrt(sig_yy), sig_yy]])
#         for rho in np.linspace(-0.999, 0.999, num=num_kernel)
#     ])
#     kernels = gaussain_full_kernels(cov_2x2_tensor, 31)
    
#     fig, axs = plt.subplots(nrows=1, ncols=num_kernel)
#     for idx, ax in enumerate(axs):
#         P.plot_scalarmap(ax, kernels[idx].cpu())
