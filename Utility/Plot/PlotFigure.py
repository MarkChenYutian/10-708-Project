...
def plot_flow_calibcurve(mean_cov_performance: FlowCovPerformance, file_name: str):
    GRID_SHAPE = (1, 2)
    _ = plt.figure(figsize=(16, 6), dpi=300)
    
    fields = ["uncertainty_avg_u", "uncertainty_avg_v"]
    for i, field in enumerate(fields):
        ax = plt.subplot2grid(GRID_SHAPE, (i // 2, i % 2), rowspan=1, colspan=1)
        ax.set_title(f"Distribution {field}", loc="left")
        ax.bar(x=[v + (5/2) for v in FlowCovPerformance.uncertainty_bin[:-2]], height=getattr(mean_cov_performance, field)[:-1], width=5., color=getColor('-', 4, 0))
        ax.plot(FlowCovPerformance.uncertainty_bin[:-2], FlowCovPerformance.uncertainty_bin[:-2], linestyle="--", zorder=2., color="orange")
        ax.set_aspect('equal', adjustable='box')
        ax.set_xlabel("Estimated Variance")
        ax.set_ylabel("Mean Error^2")
...
def plot_flow_performance(flow_results: list[FlowPerformance], file_name: str):
    ...
    for i, field in enumerate(fields):
        ...
        ax = plt.subplot2grid(GRID_SHAPE, (i // 2, i % 2), rowspan=1, colspan=1)
        ax.set_title(f"Distribution {field}", loc="left")
        plot_histogram(ax, row, bin_width=2)
...
def plot_depth_calibcurve(mean_cov_performance: DepthCovPerformance, file_name: str):
    GRID_SHAPE = (1, 1)
    _ = plt.figure(figsize=(16, 6), dpi=300)
    
    ax: plt.Axes
    field = "uncertainty_avg"
