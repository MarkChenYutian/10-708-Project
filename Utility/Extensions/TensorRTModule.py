...
class TensorRTModule(torch.nn.Module):
    T_SUPPORT_DTYPE = Literal["fp32", "fp16", "bf16"]
    ...
    def compile_torch_module(
        ...
        dtype: "TensorRTModule.T_SUPPORT_DTYPE" = "fp32",
        ...
        * `dtype` - Compile the model with to certain data type (fp32, bf16, fp16) using TensorRT engine. Using this option does
                *not* require converting module as that dtype prior to compiling. The TRT engine is designed to convert
                dtypes automatically. However, compiling to a data type that is different from training may hurt the performance.
                
        * `use_cache` - Use ONNX and TensorRT engine cache whenever available. This can drasticly reduce compile time after
                the network is compiled once on the specific platform. (No need to re-compile, just export ONNX and find whether
                the network architecture is changed is sufficient)
        """
        if cache_to.exists():
        ...
        # NOTE: PyTorch export ONNX have bug (as of Sept. 2024) Exporting a bf16 module will raise RuntimeError
        # "Tensor has no device".
        module_to_compile = module.float().cuda().eval()
        float_inputs      = tuple(inp.float() for inp in inputs)
        
        network_hash, temp_path = cls._get_module_onnx_md5(cache_box, module_to_compile, input_names, output_names, float_inputs, dynamic_axis)
        ...
        cache_box = cls._select_compiled_engine_cache(cache_box, test_profiles, input_names, dtype, network_hash)
        # End
        
        # Export as ONNX
        onnx_cache_path = cache_box.path("export_network.onnx")
        if onnx_cache_path.exists() and use_cache:
            Logger.write("info", f"Using cache at {onnx_cache_path}")
        else:
            Logger.write("info", f"Can't find cache at {onnx_cache_path}, exporting network as ONNX...")
            inference_module = module_to_compile
            
            if temp_path.exists():
                shutil.move(temp_path, cache_box.path("export_network.onnx"))
            else:
                torch.onnx.export(
                    inference_module, float_inputs,
        ...
        if trt_cache_path.exists() and use_cache:
            ...
            Logger.write("info", f"Compiling as {dtype} module.")
            match dtype:
                case "fp32": pass
                case "fp16": config.set_flag(trt.BuilderFlag.FP16)
                case "bf16": config.set_flag(trt.BuilderFlag.BF16)
                case _: raise ValueError(f"Unrecognized dtype for TensorRTModule: {dtype}")
    ...
    def _select_compiled_engine_cache(
        ...
        dtype: "TensorRTModule.T_SUPPORT_DTYPE",
        onnx_md5_hash: str,
    ) -> Sandbox:
        profile_string = "\n".join(
        ...
        for child_box in cachebox.get_children():
            ...
            if not hasattr(child_box.config, "dtype"): continue
            ...
            if  child_box.config.EngineProfile == profile_string and\
                ...
                child_box.config.dtype == dtype and\
                child_box.config.ONNXmd5 == onnx_md5_hash:
                return child_box
        
        child_box = cachebox.new_child(f"EngineProfile_{len(cachebox.get_children())}")
        child_box.config = {"EngineProfile": profile_string, "DeviceName": device_name, "dtype": dtype,
    ...
    def _get_module_onnx_md5(
        ...
        return onnx_hash, cache_box.path("temp.onnx")
