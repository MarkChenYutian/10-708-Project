#type: ignore   # Too much type error to fix : )
"""
For Flow resizing of all kinds.
Adapted from the MatchAnything repository.
"""
...
class ResizeHorizontalAxisManipulation(ImagePairsManipulationBase):
    ...
    def __call__(
        ...
        # FIXME: Yuchen: how this deals with uint8 image is shit code.
        img0_resized = (
            F.interpolate(
...
def unmap_predicted_flow(
    ...
    target_coordinates_valid = (
        F.interpolate(
            flow_valid2valid.float(),
            size=source_valid_shape.tolist(),
            mode="nearest",
            # align_corners=False
        )
...
def unmap_covariance(
    ...
    x_ratio = (img0_region_source[1] - img0_region_source[0]) / (img0_region_representation[1] - img0_region_representation[0])
    y_ratio = (img0_region_source[3] - img0_region_source[2]) / (img0_region_representation[3] - img0_region_representation[2])
    channel_output[:, 0] *= x_ratio.square()
    channel_output[:, 1] *= y_ratio.square()
    channel_output[:, 2] *= x_ratio * y_ratio
