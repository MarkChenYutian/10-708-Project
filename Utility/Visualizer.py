...
from typing import Callable, Optional, TypeVar, ParamSpec, Concatenate, Generic, Any, Sequence, TYPE_CHECKING
...
if TYPE_CHECKING:
    # Simply for static type checking & type hint. Avoid cyclic import.
...
class RerunVisualizer:
                    ...
                    rr.send_blueprint(rr.blueprint.Blueprint(
                        rr.blueprint.Horizontal(
                            ...
                            rr.blueprint.Tabs(
                                ...
                                rr.blueprint.Spatial2DView(origin="/world/Frame/cam")
                            )
    ...
    def visualizeAllPoints(gmap: TensorMap, show_cov: bool = False):
        ...
        rr.log("/world/Points", rr.Points3D(point_3d.numpy(), colors=point_color))
    ...
    @checkIsActive
    @staticmethod
    def visualizeRecentPoints(gmap: TensorMap, index=-1, use_axis_cov: bool=False):
        points = gmap.get_frame_points(gmap.frames[index])
        obs    = gmap.get_frame_observes(gmap.frames[index])
        ...
        if use_axis_cov:
            ...
            delta = points.position.repeat(1, 3, 1).reshape(-1, 3)
            ...
            rr.log(
                "/world/PointsVar",
                rr.LineStrips3D(
                    torch.stack([eigen_vec_Tw_a, eigen_vec_Tw_b], dim=1).cpu().numpy(),
                    radii=[0.003],
                    colors=[[200, 200, 10]],
                ),
            )
            rr.log("/world/Points", rr.Points3D(points.position.numpy(), colors=points.color.numpy()))
        else:
            radii  = (obs.cov_Tc.det().sqrt() * 1e2).clamp(min=0.03, max=0.5)
            rr.log("/world/Points", rr.Points3D(points.position.numpy(), colors=points.color.numpy(), radii=radii.numpy()))
    ...
    def visualizePointCov(gmap: TensorMap):
        ...
        rr.log("/world/PointsCov", rr.Points3D(gmap.points.position.numpy(), colors=gmap.points.color.numpy()))
    ...
    def visualizeImageOnCamera(image: torch.Tensor | np.ndarray, subpath: str | None = None, apply_cmap: bool = False, vmax: float | None = None):
        ...
        rr.log(f"/world/Frame/cam/{subpath}", rr.Image(np_image).compress())
    ...
    def visualizePoints(path: str, positions: np.ndarray, colors: np.ndarray | None | tuple[float, float, float] = None, radii: float | np.ndarray=0.05):
        rr.log(f"/world/{path}", rr.Points3D(positions, colors=colors, radii=radii))
...
class PLTVisualizer:
    ...
    def visualize_flow(flow_uv: torch.Tensor):
        ...
        Plot.plot_flow(ax, flow_uv[0].cpu())
        fig.tight_layout()
        return fig
    ...
    @AddVisualizerContext
    @staticmethod
    def visualize_imatcher(output: IMatcher.Output, frame0: SourceDataFrame, frame1: SourceDataFrame, slow: bool=True) -> FigureBase:
        """
        slow: if set to True, then some slow operation (plot) will be generated. For instance the cumulative distribution of epe on each axis.
        """
        GRID_SHAPE = (4, 4)
        ax: Axes
        ...
        ax.set_title(f"Frame {frame0.meta.idx}", loc="left")
        Plot.plot_image(ax, frame0.imageL.cpu().permute(0, 2, 3, 1)[0])
        
        ax = plt.subplot2grid(GRID_SHAPE, (1, 0))
        ax.set_title(f"Frame {frame1.meta.idx}", loc="left")
        Plot.plot_image(ax, frame1.imageL.cpu().permute(0, 2, 3, 1)[0])
        
        
        # Plot flows (and their masks)
        ax = plt.subplot2grid(GRID_SHAPE, (0, 1))
        ax.set_title(f"Predict flow", loc="left")
        Plot.plot_flow(ax, output.flow[0].detach().cpu())
        
        ax = plt.subplot2grid(GRID_SHAPE, (1, 1))
        ax.set_title(f"Ground truth flow", loc="left")
        if frame0.gtFlow is None: ax.set_axis_off()
        else: Plot.plot_flow(ax, frame0.gtFlow[0].cpu())
        
        ax = plt.subplot2grid(GRID_SHAPE, (0, 2))
        ax.set_title(f"Predict flow (GTMask)", loc="left")
        if frame0.flowMask is None: ax.set_axis_off()
        else:
            masked_flow = output.flow.clone().cpu()
            masked_flow[~frame0.flowMask.cpu().repeat(1, 2, 1, 1)] = 0.
            Plot.plot_flow(ax, masked_flow[0])
        
        ax = plt.subplot2grid(GRID_SHAPE, (1, 2))
        ax.set_title(f"Ground truth flow (GTMask)", loc="left")
        if frame0.flowMask is None: ax.set_axis_off()
        else:
            masked_flow = output.flow.clone().cpu()
            masked_flow[~frame0.flowMask.cpu().repeat(1, 2, 1, 1)] = 0.
        ...
        ax = plt.subplot2grid(GRID_SHAPE, (0, 3))
        ax.set_title(f"Predict flow (Pred Mask)", loc="left")
        if output.mask is None: ax.set_axis_off()
        else:
            masked_flow = output.flow.clone().cpu()
            masked_flow[~output.mask.cpu().repeat(1, 2, 1, 1)] = 0.
            Plot.plot_flow(ax, masked_flow[0])
        
        ax = plt.subplot2grid(GRID_SHAPE, (1, 3))
        ax.set_title(f"Ground truth flow (Pred Mask)", loc="left")
        if output.mask is None: ax.set_axis_off()
        else:
            masked_flow = output.flow.clone().cpu()
            masked_flow[~output.mask.cpu().repeat(1, 2, 1, 1)] = 0.
        ...
        # Plot Flow Covariance (if exists)
        ax_u = plt.subplot2grid(GRID_SHAPE, (2, 0))
        ax_v = plt.subplot2grid(GRID_SHAPE, (3, 0))
        ax_u.set_title(f"Pred cov_u (log10)", loc="left")
        ax_v.set_title(f"Pred cov_v (log10)", loc="left")
        if output.cov is None:
            ax_u.set_axis_off()
            ax_v.set_axis_off()
        else:
            plot_cov =  output.cov.log10()
            v_min, v_max = plot_cov.min().item(), plot_cov.max().item()
            im_u = Plot.plot_scalarmap(ax_u, plot_cov[0, 0], vmin=v_min, vmax=v_max)
            im_v = Plot.plot_scalarmap(ax_v, plot_cov[0, 1], vmin=v_min, vmax=v_max)
            fig.colorbar(im_u, ax=ax_u)
            fig.colorbar(im_v, ax=ax_v)
        
        
        ax_u = plt.subplot2grid(GRID_SHAPE, (2, 1))
        ax_v = plt.subplot2grid(GRID_SHAPE, (3, 1))
        ax_u.set_title(f"Pred cov_u (log, Pred Mask)", loc="left")
        ax_v.set_title(f"Pred cov_v (log, Pred Mask)", loc="left")
        if output.cov is None or output.mask is None:
            ax_u.set_axis_off()
            ax_v.set_axis_off()
        else:
            plot_cov =  output.cov.log10()
            plot_cov[~output.mask.repeat(1, 2, 1, 1)] = 0.
            v_min, v_max = 0., plot_cov.max().item()
            plot_cov[~output.mask.repeat(1, 2, 1, 1)] = torch.nan
        ...
        # Plot epe (if applicable)
        ax_u = plt.subplot2grid(GRID_SHAPE, (2, 2))
        ax_v = plt.subplot2grid(GRID_SHAPE, (3, 2))
        ax_u.set_title(f"EPE u", loc="left")
        ax_v.set_title(f"EPE v", loc="left")
        if frame0.gtFlow is None:
            ax_u.set_axis_off()
            ax_v.set_axis_off()
        else:
            plot_epe = (output.flow.cpu() - frame0.gtFlow).abs()
            v_min, v_max = plot_epe.min().item(), plot_epe.max().item()
            im_u = Plot.plot_scalarmap(ax_u, plot_epe[0, 0], vmin=v_min, vmax=v_max)
            im_v = Plot.plot_scalarmap(ax_v, plot_epe[0, 1], vmin=v_min, vmax=v_max)
            fig.colorbar(im_u, ax=ax_u)
            fig.colorbar(im_v, ax=ax_v)
        
        
        ax_u = plt.subplot2grid(GRID_SHAPE, (2, 3))
        ax_v = plt.subplot2grid(GRID_SHAPE, (3, 3))
        ax_u.set_title(f"EPE u (Pred Mask)", loc="left")
        ax_v.set_title(f"EPE v (Pred Mask)", loc="left")
        if frame0.gtFlow is None or output.mask is None:
            ax_u.set_axis_off()
            ax_v.set_axis_off()
        else:
            plot_epe = (output.flow.cpu() - frame0.gtFlow).abs()
            plot_epe[~output.mask.repeat(1, 2, 1, 1)] = 0.
            v_min, v_max = plot_epe.min().item(), plot_epe.max().item()
            plot_epe[~output.mask.repeat(1, 2, 1, 1)] = torch.nan
